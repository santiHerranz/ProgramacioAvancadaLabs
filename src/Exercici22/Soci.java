...
public class Soci implements Comparable<Soci>{
	...
	public void addQuota(Rebut t){ // Class Soci.java
		/* No cal controlar la repeticio de rebut
		 *
		 * TODO Solucio Exercici 1.1: addQuota
		 * Encuar directament el rebut a la cua
		 * Com que el metode no pot generar excepcions, hem de fer el tractament
		 */
		try{quotes.encuar(t);}
		catch(Exception e){}
	}
	...
	public void remQuota(Rebut t) throws Exception{
		/* Llancament excepcio si el rebut no esta al magatzem
		 *
		 * TODO Solucio Exercici 1.2: remQuota
		 * Recorregut de cerca amb cua de suport
		 * Si no es troba llanca excepcio
		 */

		boolean trobat = false;
		Cua<Rebut> c = new CuaEnll<Rebut>(); //Cua de support
		while (!quotes.cuaBuida()){ // Recurregut fins a l'ultim enllacat
			Rebut aux= (Rebut)(quotes.desEncuar()); // element desencuat FOR?AR casting
			if (aux.equals(t)) // Aplicar el metode equals redefinit a la classe Rebut
					trobat=true; // no encuem l'element a la cua de suport
			else c.encuar(aux); // encuem tots els altres
		}
		quotes=c; // canviem la referencia
		if (!trobat) throw new Exception("No hi es");
